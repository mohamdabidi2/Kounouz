        ...
        if (Number(value) > 0) {
            ...
            if ((Number(value) + Number(row.deposit)) >= Number(row.PrixVente)) {
                if ((Number(value) + Number(row.deposit)) > Number(row.PrixVente)) {
                    ...
                    let d = new Date()
                    res.post('/logsRegueb.json', { log: d.toLocaleString() + " : " + row.NomCli + " payer le reste ( " + value + "Dt ) de sa réservation 'Hotel' à : " + localStorage.getItem('agent') })

                    res.put("/hotelsRegueb/" + id + '.json', { ...row, deposit: Number(mtr) + Number(value), TypeDepaiment: "(payée)" })
                ...
                res.post('/logsRegueb.json', { log: d.toLocaleString() + " : " + row.NomCli + " payer une tranche ( " + value + "Dt ) de sa réservation 'Hotel' à : " + localStorage.getItem('agent') })
