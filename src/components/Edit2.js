    ...
    const editmethode = () => {
        ...
        if (Number(value) > 0) {
            ...
            if ((Number(value) + Number(row.deposit)) >= Number(row.PrixVente)) {
                ...
                let d = new Date()
                res.post('/logsRegueb.json', { log: d.toLocaleString() + " : " + row.NomCli + " payer le reste ( " + value + "Dt ) de sa réservation 'omraa' à : " + localStorage.getItem('agent') })

                    res.put("/omraaRegueb/" + id + '.json', { ...row, deposit: Number(mtr) + Number(value), TypeDepaiment:  "(payée)" })
        ...
        routemanger(0)
                ...
                res.post('/logsRegueb.json', { log: d.toLocaleString() + " : " + row.NomCli + " payer une tranche ( " + value + "Dt ) de sa réservation 'Omraa' à : " + localStorage.getItem('agent') })
